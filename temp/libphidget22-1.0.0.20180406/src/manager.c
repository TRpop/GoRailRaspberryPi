...
sendAttachVisitor(PhidgetDeviceHandle device, const PhidgetUniqueChannelDef *ucd, int index,
  int uniqueIndex, void *ctx) {
	...
	if (isNetworkPhidget(device)) {
		...
		res = Phidget_getServerName((PhidgetHandle)device, &srvname); // initializes for matching
		if (res == EPHIDGET_OK) {
...
channelDetach(PhidgetChannelHandle channel) {
	...
	/*
	 * If attaching / detaching is happening on another thread, wait
	 */
	PhidgetLock(channel);
...
void
deviceDetach(PhidgetDeviceHandle device, int needDevicesLock) {
	PhidgetChannelHandle channel;
	...
	for (i = 0; i < PHIDGET_MAXCHILDREN; i++) {
		...
		if (child) {
			...
			deviceDetach(child, needDevicesLock);
