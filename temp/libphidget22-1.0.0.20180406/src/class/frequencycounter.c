...
PhidgetFrequencyCounter_setFrequencyCutoff(PhidgetFrequencyCounterHandle ch, double frequencyCutoff) {
	...
	TESTPTR(ch);
	TESTCHANNELCLASS(ch, PHIDCHCLASS_FREQUENCYCOUNTER);
	TESTATTACHED(ch);
	TESTRANGE(frequencyCutoff, ch->minFrequencyCutoff, ch->maxFrequencyCutoff);
...
PhidgetFrequencyCounter_reset(PhidgetFrequencyCounterHandle phid) {
	...
	TESTPTR(phid);
	TESTCHANNELCLASS(phid, PHIDCHCLASS_FREQUENCYCOUNTER);
	TESTATTACHED(phid);
