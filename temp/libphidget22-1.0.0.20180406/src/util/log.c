...
static char				*basename;						/* log file base name */
...
_writelog(mos_file_t *mf, const char *hdr, const char *msg) {
	...
	len = mos_snprintf(mbuf, sizeof (mbuf), "%s%s", hdr, msg);
	if (len >= (int)sizeof (mbuf))
		return (EPHIDGET_NOSPC);
...
_rotateLogFile(uint64_t rotatesz, int keep) {
	...
	mos_snprintf(logfile, sizeof (logfile), "%s/%s", dirname, basename);
	mos_file_close(MOS_IOP_IGNORE, &logmf);

	if (keep) {
		addLogFile(logfile, logSize, 1);
	} else {
...
PhidgetLog_loges(Phidget_LogLevel level, const char *srcname, const char *str) {
	...
	return (PhidgetLog_loge(NULL, 0, NULL, srcname, level, "%s", str));
}
...
PhidgetLog_logv(const char *file, int line, const char *func, const char *srcname, Phidget_LogLevel level,
  const char *fmt, va_list va) {
	...
	if (buf[buflen - 1] != '\n')
		buf[buflen++] = '\n';
#ifdef _WINDOWS
	// Windows style newlines on windows
	buf[buflen - 1] = '\r';
	buf[buflen++] = '\n';
#endif
	buf[buflen] = '\0';
...
#if NDEBUG
	hdrlen = mos_snprintf(hdr, sizeof (hdr), "%s [%s][%#T]:", lvlToStr(level), srcname, NULL);
#else
	const char *c;
	...
	if (file == NULL)
		...
		hdrlen = mos_snprintf(hdr, sizeof (hdr), "%s [%s][%#T %.32s+%d %s()]: ", lvlToStr(level), srcname,
		  NULL, file, line, func);
#endif /* NDEBUG */
...
scanExistingLogFiles() {
	...
	for (;;) {
		...
		if (mos_strstr(di->filename, basename) == NULL)
			continue;

		if (mos_strcmp(di->filename, basename) == 0)
			continue;
...
PhidgetLog_enable(Phidget_LogLevel level, const char *dest) {
	...
	if (dest == NULL) {
		res = mos_file_open(MOS_IOP_IGNORE, &logmf, 0, MOS_FILE_STDERR);
		if (res != 0) {
			mos_mutex_unlock(&lock);
			return (res);
		}
	...
	basename = mos_strdup(bname, NULL);
	...
	res = mos_file_open(MOS_IOP_IGNORE, &logmf, MOS_FILE_CREATE | MOS_FILE_WRITE | MOS_FILE_READ, "%s", dest);
	if (res != 0)
		goto bad;

	res = mos_file_getsize(MOS_IOP_IGNORE, logmf, &logSize);
	if (res != 0) {
		fprintf(stderr, "failed to determine log file size: '%s'", dest);
		goto bad;
	}

	res = mos_file_seek(MOS_IOP_IGNORE, logmf, logSize);
	if (res != 0) {
...
addsources:
	...
	if (dest != NULL)
		PhidgetLog_log(level, "******************** Logging enabled ******************** ");
...
runNetworkLogging(void *arg) {
	...
	while (logsrvsock != MOS_INVALID_SOCKET) {
...
#if NDEBUG
		mos_snprintf(hdr, sizeof (hdr), "%s [net:%s][%#T]:", lvlToStr(ll), srcname, NULL);
#else
		...
		if (file == NULL)
			...
			mos_snprintf(hdr, sizeof (hdr), "%s [net:%s][%#T %.32s+%d %s()]: ", lvlToStr(ll), srcname,
			  NULL, file, pconf_get32(pc, -1, "ln"), pconf_getstr(pc, "()", "fn"));
#endif /* NDEBUG */
...
PhidgetLog_enableNetwork(const char *addr, int port) {
	...
	return (enableNetworkLogging(&saddr));
}
