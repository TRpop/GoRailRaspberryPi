...
sendGENERICVINT(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_VINT_GENERIC:
		...
		case BP_SENDPACKET:
			...
			return (sendVINTDataPacket(ch, buf[0], buf + 1, getBridgePacketArrayLen(bp, 0) - 1, &bp->sent));
		default:
...
recvGENERICVINT(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	switch (ch->UCD->uid) {
	case PHIDCHUID_VINT_GENERIC:
		return (bridgeSendToChannel(ch, BP_PACKET, "%*R", len, buf));
...
static PhidgetReturnCode
sendFirmware(PhidgetChannelHandle ch, const uint8_t *data, size_t dataLen) {
	...
	do {
		...
		ret = sendVINTDataPacket(ch, VINT_BOOTLOADER_PACKET_TYPE_FIRMWAREUPGRADEDATA, buffer, 34, NULL);

		if (ret != EPHIDGET_OK) {
			logerr("Error during upgrade (sending data): 0x%08x", ret);
			return (ret);
		} else
	...
	ret = sendVINTPacket(ch, VINT_CMD_FIRMWARE_UPGRADE_DONE, (VINTPacketType)0, NULL, 0, NULL);

	if (lastProgress != 1)
		bridgeSendToChannel(ch, BP_PROGRESSCHANGE, "%g", 1.0);

	if (ret != EPHIDGET_OK)
		logerr("Error during upgrade (sending VINT_CMD_FIRMWARE_UPGRADE_DONE): 0x%08x", ret);
...
sendFIRMWARE_UPGRADE_STM8S(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_STM8S_FIRMWARE_UPGRADE_100:
		...
		case BP_SENDFIRMWARE:
			...
			res = sendFirmware(ch, base64, base64Len);
			mos_free(base64, base64Len);
			return (res);
		default:
			MOS_PANIC("Unexpected packet type");
...
sendVCP1002(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_VCP1002_VOLTAGEINPUT_100:
		...
		case BP_SETDATAINTERVAL:
			...
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_SAMPLED_SETDATAINTERVAL, buf, 2, &bp->sent));
		case BP_SETCHANGETRIGGER:
			pack32(buf, (uint32_t)(getBridgePacketDouble(bp, 0) * (1 << 24)));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_VOLTAGEINPUT_SETVOLTAGECHANGETRIGGER, buf, 4, &bp->sent));
		case BP_SETVOLTAGERANGE:
			...
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_VOLTAGEINPUT_VCP1002RANGE, buf, 1, &bp->sent));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
sendVCP1001(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_VCP1001_VOLTAGEINPUT_100:
		...
		case BP_SETVOLTAGERANGE:
			...
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_VOLTAGEINPUT_VCP1001RANGE, buf, 1, &bp->sent));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
sendVCP1000(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_VCP1000_VOLTAGEINPUT_100:
		...
		case BP_SETVOLTAGERANGE:
			...
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_VOLTAGEINPUT_VCP1000RANGE, buf, 1, &bp->sent));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
sendTMP1300(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_TMP1300_TEMPERATURESENSOR_IR_100:
		...
		case BP_SETCHANGETRIGGER:
			...
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_TEMPERATURESENSOR_SETTEMPERATURECHANGETRIGGER, buf, 4, &bp->sent));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
sendTMP1200(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_TMP1200_RESISTANCEINPUT_100:
		...
		case BP_SETCHANGETRIGGER:
			packfloat(buf, (float)getBridgePacketDouble(bp, 0));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_RESISTANCEINPUT_SETRESISTANCECHANGETRIGGER, buf, 4, &bp->sent));
		case BP_SETRTDWIRESETUP:
			buf[0] = getBridgePacketInt32(bp, 0);
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_RTD_WIRESETUP, buf, 1, &bp->sent));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	...
	case PHIDCHUID_TMP1200_TEMPERATURESENSOR_RTD_100:
		...
		case BP_SETRTDTYPE:
			buf[0] = getBridgePacketInt32(bp, 0);
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_TEMPERATURESENSOR_RTDTYPE, buf, 1, &bp->sent));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
sendTMP1101(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_TMP1101_TEMPERATURESENSOR_THERMOCOUPLE_100:
		...
		case BP_SETTHERMOCOUPLETYPE:
			buf[0] = getBridgePacketInt32(bp, 0);
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_TEMPERATURESENSOR_THERMOCOUPLETYPE, buf, 1, &bp->sent));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
recvTMP1101(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_TMP1101_TEMPERATURESENSOR_THERMOCOUPLE_100:
		switch (pkt) {
		case VINT_PACKET_TYPE_TEMPERATURESENSOR_TEMPERATURECHANGE:
			temperature = round_double((double)unpackfloat(buf), 2);
			return (bridgeSendToChannel(ch, BP_TEMPERATURECHANGE, "%g", temperature));
		case VINT_PACKET_TYPE_TEMPERATURESENSOR_SATURATION:
			return (bridgeSendToChannel(ch, BP_ERROREVENT, "%d%s", EEPHIDGET_SATURATION, "Saturation Detected."));
		default:
...
sendSTC1001(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_STC1001_STEPPER_100:
		...
		case BP_SETTARGETPOSITION:
			...
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_MOTORCONTROLLER_SETTARGETPOSITION, buf, 8, &bp->sent));
		case BP_SETVELOCITYLIMIT:
			pack32(buf, (int)(getBridgePacketDouble(bp, 0) /* (1 << 7)*/));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_MOTORCONTROLLER_SETVELOCITYLIMIT, buf, 4, &bp->sent));
		case BP_SETACCELERATION:
			pack32(buf, (int)(getBridgePacketDouble(bp, 0) * (1 << 7)));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_MOTORCONTROLLER_SETACCELERATION, buf, 4, &bp->sent));
		case BP_SETCURRENTLIMIT:
			pack16(buf, (int)(getBridgePacketDouble(bp, 0) * (1 << 8)));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_STEPPER_SETCURRENTLIMIT, buf, 2, &bp->sent));
		case BP_SETHOLDINGCURRENTLIMIT:
			pack16(buf, (int)(getBridgePacketDouble(bp, 0) * (1 << 8)));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_STEPPER_SETHOLDINGCURRENTLIMIT, buf, 2, &bp->sent));
		case BP_SETENGAGED:
			buf[0] = getBridgePacketInt32(bp, 0) ? 0xFF : 0x00;
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_STEPPER_SETENGAGED, buf, 1, &bp->sent));
		case BP_SETCONTROLMODE:
			buf[0] = getBridgePacketInt32(bp, 0);
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_STEPPER_SETCONTROLMODE, buf, 1, &bp->sent));
#if 0 // XXX - is this removed from the Firmware?
		case BP_SETFAILSAFEMODE:
			buf[0] = getBridgePacketInt32(bp, 0);
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_STEPPER_SETFAILSAFEMODE, buf, 1, &bp->sent));
		case BP_SETFAILURETIMEOUT:
			doubleToSignedFixedPoint(getBridgePacketInt32(bp, 0), buf, 0, 4, 0);
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_STEPPER_SETFAILURETIMEOUT, buf, 4, &bp->sent));
#endif
		case BP_SETDATAINTERVAL:
			pack16(buf, getBridgePacketUInt32(bp, 0));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_SAMPLED_SETDATAINTERVAL, buf, 2, &bp->sent));
...
sendSND1000(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_SND1000_SOUNDSENSOR_100:
		...
		case BP_SETSPLRANGE:
			buf[0] = getBridgePacketInt32(bp, 0);
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_SOUNDSENSOR_SPLRANGE, buf, 1, &bp->sent));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	...
	case PHIDCHUID_SND1000_SOUNDSENSOR_100:
		...
		case VINT_PACKET_TYPE_SOUNDSENSOR_DBCHANGE:
			...
			if (saturatedFlag) {
				...
				dBC = 10 * log10(dBC);
				for (i = 0; i < 10; i++)
					finaloctaves[i] = octaves[9 - i]; //flip so element 0 is lowest freq band
...
sendSAF1000(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_SAF1000_POWERGUARD_100:
		...
		case BP_SETENABLED:
			buf[0] = getBridgePacketInt32(bp, 0) ? 0xFF : 0x00;
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_POWERGUARD_SETPOWERENABLED, buf, 1, &bp->sent));
		case BP_SETOVERVOLTAGE:
			overVoltage_fixed = ((int)((getBridgePacketDouble(bp, 0))* (1 << 24)));
			pack32(buf, overVoltage_fixed);
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_POWERGUARD_SETOVERVOLTAGE, buf, 4, &bp->sent));
		case BP_SETFANMODE:
			buf[0] = getBridgePacketInt32(bp, 0);
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_POWERGUARD_SETFANMODE, buf, 1, &bp->sent));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
sendREL1101(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_REL1101_DIGITALOUTPUT_100:
		...
		case BP_SETSTATE:
			...
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_DIGITALOUTPUT_SETDUTYCYCLE, buf, 1, &bp->sent));
...
sendREL1100(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_REL1100_DIGITALOUTPUT_100:
		...
		case BP_SETSTATE:
			...
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_DIGITALOUTPUT_SETDUTYCYCLE, buf, 2, &bp->sent));
		case BP_SETDUTYCYCLE:
...
sendRCC1000(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_RCC1000_RCSERVO_100:
		...
		case BP_SETENGAGED:
			buf[0] = getBridgePacketInt32(bp, 0) ? 0xFF : 0x00;
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_RCSERVO_SETENABLED, buf, 1, &bp->sent));
		case BP_SETVOLTAGE:
			buf[0] = getBridgePacketInt32(bp, 0);
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_RCSERVO_VOLTAGE, buf, 1, &bp->sent));
		case BP_SETMINPULSEWIDTH:
			pack32(buf, (uint32_t)(getBridgePacketDouble(bp, 0) * 1000));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_RCSERVO_SETMINPULSEWIDTH, buf, 4, &bp->sent));
		case BP_SETMAXPULSEWIDTH:
			pack32(buf, (uint32_t)(getBridgePacketDouble(bp, 0) * 1000));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_RCSERVO_SETMAXPULSEWIDTH, buf, 4, &bp->sent));
		case BP_SETACCELERATION:
			pack32(buf, (uint32_t)(getBridgePacketDouble(bp, 0) * 16.0 / 2500.0));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_RCSERVO_SETACCELERATION, buf, 4, &bp->sent));
		case BP_SETVELOCITYLIMIT:
			pack32(buf, (uint32_t)(getBridgePacketDouble(bp, 0) * 16.0 / 50.0));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_RCSERVO_SETVELOCITYLIMIT, buf, 4, &bp->sent));
		case BP_SETTARGETPOSITION:
			pack32(buf, (uint32_t)(getBridgePacketDouble(bp, 0) * 1000));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_RCSERVO_SETTARGETPOSITION, buf, 4, &bp->sent));
		case BP_SETSPEEDRAMPINGSTATE:
			pack32(buf, getBridgePacketInt32(bp, 0));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_RCSERVO_SETSPEEDRAMPINGSTATE, buf, 4, &bp->sent));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
static PhidgetReturnCode
sendVINTSERVO_S2313M(PhidgetChannelHandle ch, BridgePacket *bp) {
	uint8_t buf[8];
	...
	switch (ch->UCD->uid) {
	case PHIDCHUID_VINTSERVO_S2313M_SERVO_100:
		switch (bp->vpkt) {
		case BP_SETENGAGED:
			buf[0] = getBridgePacketInt32(bp, 0) ? 0xFF : 0x00;
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_RCSERVO_SETENABLED, buf, 1, &bp->sent));
		case BP_SETDUTYCYCLE:
			pack16(buf, (int16_t)(getBridgePacketDouble(bp, 0) * 1024));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_RCSERVO_DUTYCYCLELIMIT, buf, 2, &bp->sent));
		case BP_SETTARGETPOSITION:
			pack32(buf, (uint32_t)(getBridgePacketDouble(bp, 0)));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_RCSERVO_SETTARGETPOSITION, buf, 4, &bp->sent));
		case BP_SETSPEEDRAMPINGSTATE:
			buf[0] = (getBridgePacketInt32(bp, 0) != 0);
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_RCSERVO_SETSPEEDRAMPINGSTATE, buf, 1, &bp->sent));
		case BP_SETVELOCITYLIMIT:
			pack32(buf, (uint32_t)(getBridgePacketDouble(bp, 0)));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_RCSERVO_SETVELOCITYLIMIT, buf, 4, &bp->sent));
		case BP_SETACCELERATION:
			pack32(buf, (int32_t)(getBridgePacketDouble(bp, 0)));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_RCSERVO_SETACCELERATION, buf, 4, &bp->sent));
		case BP_SETDATAINTERVAL:
...
static PhidgetReturnCode
recvVINTSERVO_S2313M(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	PhidgetReturnCode res;
	unsigned int motorPosition;
	int pkt;
	...
	case PHIDCHUID_VINTSERVO_S2313M_SERVO_100:
		switch (pkt) {
		case VINT_PACKET_TYPE_RCSERVO_ARRIVED_AT_TARGET_POSITION:
			motorPosition = unpack32(buf);
			res = bridgeSendToChannel(ch, BP_POSITIONCHANGE, "%g", (double)motorPosition);
			if (res != EPHIDGET_OK)
				return res;
			return (bridgeSendToChannel(ch, BP_TARGETPOSITIONREACHED, "%g", (double)motorPosition));
		case VINT_PACKET_TYPE_RCSERVO_MOTORSTATUSUPDATE:
			motorPosition = unpack32(buf);
			return (bridgeSendToChannel(ch, BP_POSITIONCHANGE, "%g", (double)motorPosition));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
static PhidgetReturnCode
sendVINTSERVO_S3317M(PhidgetChannelHandle ch, BridgePacket *bp) {
	uint8_t buf[8];
	...
	switch (ch->UCD->uid) {
	case PHIDCHUID_VINTSERVO_S3317M_SERVO_100:
		switch (bp->vpkt) {
		case BP_SETENGAGED:
			buf[0] = getBridgePacketInt32(bp, 0) ? 0xFF : 0x00;
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_RCSERVO_SETENABLED, buf, 1, &bp->sent));
		case BP_SETDUTYCYCLE:
			pack16(buf, (int16_t)((getBridgePacketDouble(bp, 0) * 512) + 256)); //range from 25% to 75% of 1024
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_RCSERVO_DUTYCYCLELIMIT, buf, 2, &bp->sent));
		case BP_SETTARGETPOSITION:
			pack32(buf, (uint32_t)(getBridgePacketDouble(bp, 0)));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_RCSERVO_SETTARGETPOSITION, buf, 4, &bp->sent));
...
static PhidgetReturnCode
recvVINTSERVO_S3317M(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	PhidgetReturnCode res;
	unsigned int motorPosition;
	int pkt;
...
static PhidgetReturnCode
sendVINTSERVO_S4309M(PhidgetChannelHandle ch, BridgePacket *bp) {
	uint8_t buf[8];
	...
	switch (ch->UCD->uid) {
	case PHIDCHUID_VINTSERVO_S4309M_SERVO_100:
		switch (bp->vpkt) {
		case BP_SETENGAGED:
			buf[0] = getBridgePacketInt32(bp, 0) ? 0xFF : 0x00;
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_RCSERVO_SETENABLED, buf, 1, &bp->sent));
...
static PhidgetReturnCode
recvVINTSERVO_S4309M(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	PhidgetReturnCode res;
	unsigned int motorPosition;
	int pkt;
...
static PhidgetReturnCode
sendVINTSERVO_S4309R(PhidgetChannelHandle ch, BridgePacket *bp) {
	uint8_t buf[8];
	...
	case PHIDCHUID_VINTSERVO_S4309R_DCMOTOR_100:
		switch (bp->vpkt) {
		case BP_SETACCELERATION:
		...
		case BP_SETDUTYCYCLE:
			...
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_DCMOTOR_SETVELOCITY, buf, 4, &bp->sent));
		case BP_SETBRAKINGDUTYCYCLE:
			pack32(buf, (int32_t)((getBridgePacketDouble(bp, 0)) * 2097151)); //have to multiply by 959 so save 10 bits for multiplication in firmware
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_DCMOTOR_SETBRAKINGSTRENGTH, buf, 4, &bp->sent));
		case BP_SETCURRENTLIMIT:
			packfloat(buf, (float)getBridgePacketDouble(bp, 0));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_DCMOTOR_SETCURRENTLIMIT, buf, 4, &bp->sent));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
static PhidgetReturnCode
recvVINTSERVO_S4309R(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	double dutyCycle, sign;
	uint16_t tempu16;
	...
	case PHIDCHUID_VINTSERVO_S4309R_DCMOTOR_100:
		...
		case VINT_PACKET_TYPE_DCMOTOR_MOTORSTATUSUPDATE:
			tempu16 = unpack16(buf);
			//braking = (tempu16 & 0x8000) ? PTRUE : PFALSE; // Not Used
			sign = (tempu16 & 0x4000) ? -1 : 1;
			tempu16 &= 0x3FFF; //clear braking flag and sign flag
			dutyCycle = (double)unpack16(buf + 2);
...
sendPRE1000(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_PRE1000_PRESSURESENSOR_100:
		...
		case BP_SETCHANGETRIGGER:
			pack32(buf, (uint32_t)(getBridgePacketDouble(bp, 0) * (1 << 16)));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_PRESSURESENSOR_SETPRESSURECHANGETRIGGER, buf, 4, &bp->sent));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
sendMOT1101_SETCORRECTIONPARAMETERS(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	res = sendVINTDataPacketTransaction(ch, VINT_PACKET_TYPE_MAGNETOMETER_SET_CORRECTION_PARAMETERS, buf, 28, &trans, &bp->sent);
	if (res != EPHIDGET_OK)
		goto setcorrectionparams_done;
	...
	res = sendVINTDataPacketTransaction(ch, VINT_PACKET_TYPE_MAGNETOMETER_SET_CORRECTION_PARAMETERS, buf, 24, &trans, &bp->sent);
...
sendMOT1101(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_MOT1101_SPATIAL_100:
		...
		case BP_SETDATAINTERVAL:
			...
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_SAMPLED_SETDATAINTERVAL, buf, 2, &bp->sent));
		case BP_ZERO:
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_GYROSCOPE_ZERO, NULL, 0, &bp->sent));
		case BP_RESETCORRECTIONPARAMETERS:
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_MAGNETOMETER_RESET_CORRECTION_PARAMETERS, NULL, 0, &bp->sent));
		case BP_SAVECORRECTIONPARAMETERS:
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_MAGNETOMETER_SAVECORRECTIONPARAMETERS, NULL, 0, &bp->sent));
	...
	case PHIDCHUID_MOT1101_MAGNETOMETER_100:
		...
		case BP_SETCHANGETRIGGER:
			packfloat(buf, (float)(getBridgePacketDouble(bp, 0)));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_AXES_SETAXISCHANGETRIGGER, buf, 4, &bp->sent));
		case BP_RESETCORRECTIONPARAMETERS:
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_MAGNETOMETER_RESET_CORRECTION_PARAMETERS, NULL, 0, &bp->sent));
		case BP_SAVECORRECTIONPARAMETERS:
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_MAGNETOMETER_SAVECORRECTIONPARAMETERS, NULL, 0, &bp->sent));
...
sendMOT1100_OLD(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_MOT1100_ACCELEROMETER_100:
		...
		case BP_SETCHANGETRIGGER:
			...
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_AXES_SETAXISCHANGETRIGGER, buf, 2, &bp->sent));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
sendLUX1000(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_LUX1000_LIGHTSENSOR_100:
		...
		case BP_SETCHANGETRIGGER:
			...
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_LIGHTSENSOR_SETILLUMINANCECHANGETRIGGER, buf, 4, &bp->sent));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	...
	case PHIDCHUID_LED1000_DIGITALOUTPUT_100:
		...
		case BP_SETLEDFORWARDVOLTAGE:
			...
			case LED_FORWARD_VOLTAGE_5_6V:
				return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_DIGITALOUTPUT_LEDFORWARDVOLTAGE, buf, 1, &bp->sent));
			default:
				return (EPHIDGET_INVALIDARG);
			}
		case BP_SETLEDCURRENTLIMIT:
			pack32(buf, (uint32_t)(getBridgePacketDouble(bp, 0) * (1 << 16)));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_DIGITALOUTPUT_LEDCURRENTLIMIT, buf, 4, &bp->sent));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
sendLCD1100_WRITETEXT(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	while (numsent < numchars) {
		...
		xpos += count*xsize;
		res = sendVINTDataPacketTransaction(ch, VINT_PACKET_TYPE_GRAPHICLCD_WRITETEXT, buf, count + 6, &trans, &bp->sent);
		if (res != EPHIDGET_OK)
			break;
	}
writetext_done:
...
sendLCD1100_WRITEBITMAP(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	res = sendVINTDataPacketTransaction(ch, VINT_PACKET_TYPE_GRAPHICLCD_WRITEBITMAP, buf, 4, &trans, &bp->sent);
	if (res != EPHIDGET_OK)
		goto writebitmap_done;
	...
	while (res == EPHIDGET_OK && x < xsize) {
		...
		res = sendVINTDataPacketTransaction(ch, VINT_PACKET_TYPE_GRAPHICLCD_BITMAPDATA, buf, count, &trans, &bp->sent);
		if (res != EPHIDGET_OK)
			goto writebitmap_done;
	}
writebitmap_done:
...
sendLCD1100_SETCHARACTERBITMAP(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	if (frameBuffer != (int)font) {
		...
		buf[0] = font;
		res = sendVINTDataPacketTransaction(ch, VINT_PACKET_TYPE_GRAPHICLCD_SETFRAMEBUFFER, buf, 1, &trans, &bp->sent);
		if (res != EPHIDGET_OK)
			goto setcharacterbitmap_done;
	}
...
sendLCD1100(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_LCD1100_LCD_100:
		...
		case BP_SAVEFRAMEBUFFER:
			...
			if (buf[0] < 0 || buf[0] > 2)
				return (EPHIDGET_INVALIDARG);
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_GRAPHICLCD_SAVEFRAMEBUFFER, buf, 1, &bp->sent));
		case BP_SETFRAMEBUFFER:
			buf[0] = getBridgePacketInt32(bp, 0);
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_GRAPHICLCD_SETFRAMEBUFFER, buf, 1, &bp->sent));
		case BP_SETBACKLIGHT:
			...
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_GRAPHICLCD_SETBACKLIGHT, buf, 1, &bp->sent));
		case BP_SETCONTRAST:
			if (getBridgePacketDouble(bp, 0) >= 1)
				buf[0] = 0xFF;
			else
				buf[0] = ((int)((getBridgePacketDouble(bp, 0)) * (1 << 8)));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_GRAPHICLCD_SETCONTRAST, buf, 1, &bp->sent));
		case BP_DRAWPIXEL:
			buf[0] = getBridgePacketInt32(bp, 0);
			buf[1] = getBridgePacketInt32(bp, 1);
			buf[2] = getBridgePacketInt32(bp, 2);
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_GRAPHICLCD_DRAWPIXEL, buf, 3, &bp->sent));
		case BP_DRAWLINE:
			buf[0] = getBridgePacketInt32(bp, 0);
			...
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_GRAPHICLCD_DRAWLINE, buf, 4, &bp->sent));
		case BP_DRAWRECT:
			buf[0] = getBridgePacketInt32(bp, 0);
			buf[1] = getBridgePacketInt32(bp, 1);
			buf[2] = getBridgePacketInt32(bp, 2);
			...
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_GRAPHICLCD_DRAWRECT, buf, 6, &bp->sent));
		case BP_WRITETEXT:
			return (sendLCD1100_WRITETEXT(ch, bp));
		case BP_WRITEBITMAP:
			return (sendLCD1100_WRITEBITMAP(ch, bp));
		case BP_FLUSH:
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_GRAPHICLCD_FLUSH, NULL, 0, &bp->sent));
		case BP_CLEAR:
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_GRAPHICLCD_CLEAR, NULL, 0, &bp->sent));
		case BP_SETSLEEP:
			buf[0] = getBridgePacketInt32(bp, 0) ? 0xFF : 0x00;
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_GRAPHICLCD_SETSLEEP, buf, 1, &bp->sent));
		...
		case BP_COPY:
			...
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_GRAPHICLCD_COPY, buf, 9, &bp->sent));
		case BP_INITIALIZE:
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_GRAPHICLCD_INIT, NULL, 0, &bp->sent));
			break;
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
sendHUM1000(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_HUM1000_HUMIDITYSENSOR_100:
		...
		case BP_SETCHANGETRIGGER:
			pack32(buf, (uint32_t)(getBridgePacketDouble(bp, 0) * (1 << 16)));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_HUMIDITYSENSOR_SETHUMIDITYCHANGETRIGGER, buf, 4, &bp->sent));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
sendHIN1100(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_HIN1100_VOLTAGERATIOINPUT_100:
		...
		case BP_SETCHANGETRIGGER:
			...
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_VOLTAGERATIOINPUT_SETVOLTAGERATIOCHANGETRIGGER, buf, 2, &bp->sent));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
sendHIN1001(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_HIN1001_CAPACITIVETOUCH_WHEEL_100:
		...
		case BP_SETCHANGETRIGGER:
			...
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_CAPACITIVETOUCH_TOUCHVALUECHANGETRIGGER, buf, 2, &bp->sent));
		case BP_SETSENSITIVITY:
			if (ch->index == 4)
				buf[0] = (uint32_t)((1 - getBridgePacketDouble(bp, 0)) * 32) + 1;
			else
				buf[0] = (uint32_t)((1 - getBridgePacketDouble(bp, 0)) * 64) + 1;
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_CAPACITIVETOUCH_SENSITIVITY, buf, 1, &bp->sent));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
sendHIN1000(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_HIN1000_CAPACITIVETOUCH_100:
		...
		case BP_SETCHANGETRIGGER:
	//		pack16(buf, (unsigned int)(getBridgePacketDouble(bp, 0) * (1 << 15)));
			return EPHIDGET_OK;// (sendVINTDataPacket(ch, VINT_PACKET_TYPE_CAPACITIVETOUCH_TOUCHVALUECHANGETRIGGER, buf, 2, &bp->sent));
		case BP_SETSENSITIVITY:
...
sendENC1000(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_ENC1000_ENCODER_100:
		...
		case BP_SETCHANGETRIGGER:
			...
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_ENCODER_SETENCODERCHANGETRIGGER, buf, 4, &bp->sent));
		case BP_SETIOMODE:
			buf[0] = getBridgePacketInt32(bp, 0);
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_ENCODER_IOMODE, buf, 1, &bp->sent));
		case BP_SETENABLED:
			buf[0] = getBridgePacketInt32(bp, 0);
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_ENCODER_ENABLE, buf, 1, &bp->sent));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
recvENC1000(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_ENC1000_ENCODER_100:
		...
		case VINT_PACKET_TYPE_ENCODER_ENCODERCHANGE:
			//TOOD: verify fixed point conversion on numbers with MSB high
...
sendDST1200(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_DST1200_DISTANCESENSOR_100:
		...
		case BP_SETCHANGETRIGGER:
			pack16(buf, getBridgePacketUInt32(bp, 0));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_DISTANCESENSOR_SETDISTANCECHANGETRIGGER, buf, 2, &bp->sent));
		case BP_SETSONARQUIETMODE:
			buf[0] = (getBridgePacketInt32(bp, 0) ? 1 : 0);
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_SONAR_QUIETMODE, buf, 1, &bp->sent));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
sendDST1000(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_DST1000_DISTANCESENSOR_100:
		...
		case BP_SETCHANGETRIGGER:
			...
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_DISTANCESENSOR_SETDISTANCECHANGETRIGGER, buf, 1, &bp->sent));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
sendDCC1100(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_DCC1100_MOTORPOSITIONCONTROLLER_100:
		...
		case BP_SETDEADBAND:
			pack32(buf, getBridgePacketUInt32(bp, 0));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_MOTORPOSITIONCONTROLLER_SETDEADBAND, buf, 4, &bp->sent));
		case BP_SETKP:
			packfloat(buf, (float)getBridgePacketDouble(bp, 0));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_MOTORPOSITIONCONTROLLER_SETKP, buf, 4, &bp->sent));
		case BP_SETKD:
			packfloat(buf, (float)getBridgePacketDouble(bp, 0));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_MOTORPOSITIONCONTROLLER_SETKD, buf, 4, &bp->sent));
		case BP_SETKI:
			packfloat(buf, (float)getBridgePacketDouble(bp, 0));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_MOTORPOSITIONCONTROLLER_SETKI, buf, 4, &bp->sent));
		case BP_SETTARGETPOSITION:
			pack64(buf, (uint64_t)getBridgePacketInt64(bp, 0));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_MOTORPOSITIONCONTROLLER_SETTARGETPOSITION, buf, 8, &bp->sent));
		case BP_SETACCELERATION:
			packfloat(buf, (float)(getBridgePacketDouble(bp, 0)));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_MOTORPOSITIONCONTROLLER_SETACCELERATION, buf, 4, &bp->sent));
		case BP_SETDUTYCYCLE:
			packfloat(buf, (float)(getBridgePacketDouble(bp, 0)));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_MOTORPOSITIONCONTROLLER_SETVELOCITYLIMIT, buf, 4, &bp->sent));
		case BP_SETBRAKINGDUTYCYCLE:
			...
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_BLDCMOTOR_SETBRAKINGSTRENGTH, buf, 4, &bp->sent));
		case BP_SETDATAINTERVAL:
			pack16(buf, getBridgePacketUInt32(bp, 0));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_SAMPLED_SETDATAINTERVAL, buf, 2, &bp->sent));
		case BP_SETENGAGED:
			buf[0] = getBridgePacketInt32(bp, 0);
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_MOTORPOSITIONCONTROLLER_SETENGAGED, buf, 1, &bp->sent));
		case BP_SETSTALLVELOCITY:
			pack32(buf, (getBridgePacketDouble(bp, 0))); //convert to commutations per second
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_MOTORPOSITIONCONTROLLER_SETSTALLVELOCITY, buf, 4, &bp->sent));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	...
	case PHIDCHUID_DCC1100_TEMPERATURESENSOR_IC_100:
		...
		case BP_SETCHANGETRIGGER:
			...
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_TEMPERATURESENSOR_SETTEMPERATURECHANGETRIGGER, buf, 2, &bp->sent));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	...
	case PHIDCHUID_DCC1100_BLDCMOTOR_100:
		...
		case BP_SETACCELERATION:
			pack32(buf, (uint32_t)((getBridgePacketDouble(bp, 0)) * 8192));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_BLDCMOTOR_SETACCELERATION, buf, 4, &bp->sent));
		case BP_SETDUTYCYCLE:
			pack32(buf, (int32_t)((getBridgePacketDouble(bp, 0)) * 2097151)); //have to multiply by 959 so save 10 bits for multiplication in firmware
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_BLDCMOTOR_SETVELOCITY, buf, 4, &bp->sent));
		case BP_SETBRAKINGDUTYCYCLE:
			pack32(buf, (uint32_t)((getBridgePacketDouble(bp, 0)) * 2097151)); //have to multiply by 959 so save 10 bits for multiplication in firmware
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_BLDCMOTOR_SETBRAKINGSTRENGTH, buf, 4, &bp->sent));
		case BP_SETCURRENTLIMIT:
		...
		case BP_SETSTALLVELOCITY:
			pack32(buf, (getBridgePacketDouble(bp, 0))); //convert to commutations per second
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_BLDCMOTOR_SETSTALLVELOCITY, buf, 4, &bp->sent));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
sendDCC1000(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_DCC1000_CURRENTINPUT_100:
		...
		case BP_SETCHANGETRIGGER:
			...
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_CURRENTINPUT_SETCURRENTCHANGETRIGGER, buf, 2, &bp->sent));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	...
	case PHIDCHUID_DCC1000_VOLTAGERATIOINPUT_100:
		...
		case BP_SETCHANGETRIGGER:
			pack32(buf, (uint32_t)(getBridgePacketDouble(bp, 0)*(1 << 16)));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_VOLTAGERATIOINPUT_SETVOLTAGERATIOCHANGETRIGGER, buf, 4, &bp->sent));
	...
	case PHIDCHUID_DCC1000_DCMOTOR_100:
		...
		case BP_SETFANMODE:
			buf[0] = getBridgePacketInt32(bp, 0);
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_MOTORCONTROLLER_FAN, buf, 1, &bp->sent));
		case BP_SETCURRENTREGULATORGAIN:
			...
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_DCMOTOR_SETCURRENTREGULATORGAIN, buf, 4, &bp->sent));
		case BP_SETDATAINTERVAL:
			pack16(buf, getBridgePacketUInt32(bp, 0));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_SAMPLED_SETDATAINTERVAL, buf, 2, &bp->sent));
		default:
...
sendDCC1000_POSITIONCONTROL(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_DCC1000_MOTORPOSITIONCONTROLLER_200:
		...
		case BP_SETIOMODE:
			buf[0] = getBridgePacketInt32(bp, 0);
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_MOTORPOSITIONCONTROLLER_SETENCODERIOMODE, buf, 1, &bp->sent));
		case BP_SETENGAGED:
			buf[0] = getBridgePacketInt32(bp, 0);
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_MOTORPOSITIONCONTROLLER_SETENGAGED, buf, 1, &bp->sent));
		default:
...
recvDCC1000(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	int indexPosition;
	uint64_t timeChange;
	int indexTriggered, positionChange;
	PhidgetReturnCode result;
	int pkt;
...
static PhidgetReturnCode
sendMCSML1(PhidgetChannelHandle ch, BridgePacket *bp) {
	uint8_t buf[8];
	...
	switch (ch->UCD->uid) {
	case PHIDCHUID_MCSML1_ENCODER_100:
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, getBridgePacketUInt32(bp, 0));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_SAMPLED_SETDATAINTERVAL, buf, 2, &bp->sent));
	...
	case PHIDCHUID_MCSML1_DCMOTOR_100:
		switch (bp->vpkt) {
		case BP_SETACCELERATION:
			pack32(buf, (uint32_t)((getBridgePacketDouble(bp, 0)) * 8192));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_MOTORCONTROLLER_SETACCELERATION, buf, 4, &bp->sent));
	...
	case PHIDCHUID_MCSML1_MOTORPOSITIONCONTROLLER_100:
		switch (bp->vpkt) {
		case BP_SETDEADBAND:
			pack32(buf, getBridgePacketUInt32(bp, 0));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_MOTORPOSITIONCONTROLLER_SETDEADBAND, buf, 4, &bp->sent));
...
static PhidgetReturnCode
recvMCSML1(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	double dutyCycle, brakingDutyCycle;
	...
	case PHIDCHUID_MCSML1_DCMOTOR_100:
		...
		case VINT_PACKET_TYPE_DCMOTOR_MOTORSTATUSUPDATE:
			...
			brakingDutyCycle = (double)((int32_t)unpack32(buf + 4));
			brakingDutyCycle = round_double(brakingDutyCycle, 3);
			dutyCycle = (double)((int32_t)unpack32(buf));
...
sendDAQ1500(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_DAQ1500_VOLTAGERATIOINPUT_100:
		...
		case BP_SETENABLED:
			buf[0] = (uint8_t)getBridgePacketInt32(bp, 0) ? 0xFF : 0x00;
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_VOLTAGERATIOINPUT_BRIDGEENABLED, buf, 1, &bp->sent));
		case BP_SETDATAINTERVAL:
			pack16(buf, getBridgePacketUInt32(bp, 0));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_SAMPLED_SETDATAINTERVAL, buf, 2, &bp->sent));
		case BP_SETCHANGETRIGGER:
		...
		case BP_SETBRIDGEGAIN:
			buf[0] = getBridgePacketInt32(bp, 0);
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_VOLTAGERATIOINPUT_BRIDGEGAIN, buf, 1, &bp->sent));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
sendVCP1100(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_VCP1100_CURRENTINPUT_100:
		...
		case BP_SETCHANGETRIGGER:
			...
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_CURRENTINPUT_SETCURRENTCHANGETRIGGER, buf, 4, &bp->sent));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
sendDAQ1400(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_DAQ1400_FREQUENCYCOUNTER_100:
		...
		case BP_SETPOWERSUPPLY:
			buf[0] = getBridgePacketInt32(bp, 0);
			return(sendVINTDataPacket(ch, VINT_PACKET_TYPE_ISA_SETPOWERSUPPLY, buf, 1, &bp->sent));
		case BP_SETINPUTMODE:
			buf[0] = getBridgePacketInt32(bp, 0);
			return(sendVINTDataPacket(ch, VINT_PACKET_TYPE_ISA_SETINPUTMODE, buf, 1, &bp->sent));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	...
	case PHIDCHUID_DAQ1400_CURRENTINPUT_100:
		...
		case BP_SETDATAINTERVAL:
			...
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_SAMPLED_SETDATAINTERVAL, buf, 2, &bp->sent));
		case BP_SETCHANGETRIGGER:
			pack32(buf, (int)((getBridgePacketDouble(bp, 0) / 1.25) * (1 << 30)));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_CURRENTINPUT_SETCURRENTCHANGETRIGGER, buf, 4, &bp->sent));
		case BP_SETPOWERSUPPLY:
			buf[0] = getBridgePacketInt32(bp, 0);
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_ISA_SETPOWERSUPPLY, buf, 1, &bp->sent));
...
sendOUT1002(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_OUT1002_VOLTAGEOUTPUT_100:
		...
		case BP_SETVOLTAGE:
			...
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_VOLTAGEOUTPUT_SETVOLTAGE, buf, 4, &bp->sent));
		case BP_SETVOLTAGERANGE:
			buf[0] = getBridgePacketInt32(bp, 0);
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_VOLTAGEOUTPUT_SETMODE, buf, 1, &bp->sent));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
sendOUT1000(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_OUT1000_VOLTAGEOUTPUT_100:
		...
		case BP_SETENABLED:
			buf[0] = (uint8_t)getBridgePacketInt32(bp, 0) ? 0xFF : 0x00;
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_VOLTAGEOUTPUT_SETENABLED, buf, 1, &bp->sent));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
static PhidgetReturnCode
sendADP1001(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_ADP1001_DATAADAPTER_100:
		switch (bp->vpkt) {
		case BP_DATAOUT:
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_SPI_DATAOUT, getBridgePacketUInt8Array(bp, 0),
				getBridgePacketArrayCnt(bp, 0), &bp->sent));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
static PhidgetReturnCode
recvADP1001(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	int pkt;
	int overrun;
	...
	case PHIDCHUID_ADP1001_DATAADAPTER_100:
		switch (pkt) {
		case VINT_PACKET_TYPE_SPI_OVERRUN:
			overrun = 1;
		case VINT_PACKET_TYPE_SPI_DATAIN:
			return (bridgeSendToChannel(ch, BP_DATAIN, "%*R%u", len - 1, buf, overrun));
		default:
			MOS_PANIC("Unexpected packet type");
		}
		break;
...
sendADP1000(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_ADP1000_VOLTAGEINPUT_100:
		...
		case BP_SETVOLTAGERANGE:
			...
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_VOLTAGEINPUT_PHRANGE, buf, 1, &bp->sent));
		case BP_SETSENSORTYPE:
			return (supportedVoltageSensorType(ch, getBridgePacketInt32(bp, 0)) ? EPHIDGET_OK : EPHIDGET_INVALIDARG);
		case BP_SETSENSORVALUECHANGETRIGGER:
			return (EPHIDGET_OK);
	...
	case PHIDCHUID_ADP1000_PHSENSOR_100:
		...
		case BP_SETCHANGETRIGGER:
			pack32(buf, (uint32_t)(getBridgePacketDouble(bp, 0) * (1 << 24)));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_PHSENSOR_SETPHCHANGETRIGGER, buf, 4, &bp->sent));
		case BP_SETCORRECTIONTEMPERATURE:
			pack16(buf, (uint16_t)(getBridgePacketDouble(bp, 0) * (1 << 3)));
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_PHSENSOR_SETCORRECTIONTEMPERATURE, buf, 2, &bp->sent));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	default:
...
sendVOLTAGEINPUT_PORT(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_HUB_VOLTAGEINPUT_100:
		...
		case BP_SETCHANGETRIGGER:
			...
			return (sendVINTDataPacket(ch, VINT_PACKET_TYPE_VOLTAGEINPUT_SETVOLTAGECHANGETRIGGER, buf, 2, &bp->sent));
		case BP_SETSENSORTYPE:
			return (supportedVoltageSensorType(ch, getBridgePacketInt32(bp, 0)) ? EPHIDGET_OK : EPHIDGET_INVALIDARG);
		case BP_SETSENSORVALUECHANGETRIGGER:
			return (EPHIDGET_OK);
