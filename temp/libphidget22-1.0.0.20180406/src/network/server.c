...
handleOpenChannel(mosiop_t iop, PhidgetNetConnHandle nc, netreq_t *req) {
	...
	if (def == NULL)
		return (sendSimpleReply(nc, req->nr_reqseq, EPHIDGET_INVALIDARG, NULL));

	if (def->version != version) {
		netlogerr("received open request for channel class 0x%x, with version %d; expected version %d",
		  cclass, version, def->version);
		return (sendSimpleReply(nc, req->nr_reqseq, EPHIDGET_BADVERSION, NULL));
	}

	res = openServerChannel(pid, cclass, index, nc, &channel, req->nr_reqseq);
	if (res != EPHIDGET_OK)
		return (sendSimpleReply(nc, req->nr_reqseq, res, NULL));
	else
...
sendEventToEachClient(mosiop_t iop, msgtype_t type, msgsubtype_t subtype, void *buf, uint32_t len, int dgram) {
	...
	MTAILQ_FOREACH(nc, &openServers, openlink) {
		...
		if (res != EPHIDGET_OK)
			netlogerr("sendEventToClient() failed [%s] for %s", mos_notice_name(res), nc->peername);
	}
	mos_mutex_unlock(&srvlock);
...
sendNetDeviceAttached(PhidgetDeviceHandle device, PhidgetNetConnHandle nc) {
	...
	len = mkJSON(bufp, len, "{type:%s,"
	  ...
	  device->deviceInfo.UDD->name, deviceInfo(device, desc, sizeof(desc)), device->fwstr);
...
sendDevicesToNewServer(PhidgetNetConnHandle nc) {
	...
	FOREACH_DEVICE(device)
		if (device->connType == PHIDCONN_USB || device->connType == PHIDCONN_SPI || device->connType == PHIDCONN_LIGHTNING)
...
handleDeviceRequest(mosiop_t iop, PhidgetNetConnHandle nc, netreq_t *req, int *stop) {
	...
	*stop = 0;
	...
	return (MOS_ERROR(iop, EPHIDGET_UNEXPECTED, "unexpected msg type:%d/%d", req->nr_type, req->nr_stype));
}
...
sendKeepAlive(PhidgetNetConnHandle nc) {
	...
	netlogverbose("sending keepalive (%s)", nc->peername);
