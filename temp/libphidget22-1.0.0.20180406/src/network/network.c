...
handleReply(PhidgetNetConnHandle nc, netreq_t *req) {
	...
	if (req->nr_type == MSG_COMMAND && req->nr_stype == SMSG_KEEPALIVE) {
		netlogverbose("keepalive reply received (%s)\n", nc->peername);
		if (nc->server) {
...
PhidgetReturnCode
simpleWaitForReply(WaitForReply **_wfr, PhidgetReturnCode *rres, char **ureply) {
	PhidgetReturnCode res;
	WaitForReply *wfr;
	uint32_t cnt;
	char *reply;
	...
	err = parseJSON((char *)wfr->req.nr_data, wfr->req.nr_len, NULL, 0, "%O,E=%uR?=%s", &cnt, rres, &reply);
	if (err == 3 && reply != NULL && ureply != NULL) {
		*ureply = reply;
	} else if (reply != NULL) {
...
serverAuthenticateClient(mosiop_t iop, const char *srvname, const char *passwd, PhidgetNetConnHandle nc) {
	...
	if (req.nr_type != MSG_CONNECT || req.nr_stype != SMSG_AUTHC0)
		return (MOS_ERROR(iop, EPHIDGET_INVALID, "client did not send an auth C0 request (%d/%d)",
			req.nr_type, req.nr_stype));
	...
	*p = '\0';
	...
	if (req.nr_type != MSG_CONNECT || req.nr_stype != SMSG_AUTHC1)
		return (MOS_ERROR(iop, EPHIDGET_INVALID, "client did not send an auth C1 request (%d/%d)",
			req.nr_type, req.nr_stype));
...
startServerConnection(mosiop_t iop, IPhidgetServerHandle server) {
	...
	if (req.nr_type != MSG_CONNECT || req.nr_stype != SMSG_HANDSHAKEC0)
		return (MOS_ERROR(iop, EPHIDGET_INVALID, "client did not send a handshake request (%d/%d)",
			req.nr_type, req.nr_stype));
