...
/* USB */
	if (mos_strcasecmp(type, "USB") == 0) {
		...
		if (res != EPHIDGET_OK) {
			PhidgetRelease(&phid);
			return (MOS_ERROR(iop, res, "failed to attached network device "));
		}
...
/* VINT */
		...
		res = addNetVintDevice(phid, index, vintID, version, label, hubPort, pid, nc, &vint);
		PhidgetRelease(&phid);
		if (res != EPHIDGET_OK)
...
handleBridgePacket(mosiop_t iop, PhidgetNetConnHandle nc, netreq_t *req) {
	...
	netlogverbose("%"PRIphid": %d/%d", nc, req->nr_type, req->nr_stype);

	res = parseBridgePacketJSON(nc->tokens, &bp, (char *)req->nr_data, req->nr_len);
	if (res != EPHIDGET_OK) {
		netlogerr("client failed to parse bridge packet:%s\n", mos_notice_name(res));
		return (MOS_ERROR(iop, EPHIDGET_UNEXPECTED, "invalid json in device attach"));
	}
...
handleCommandMessage(mosiop_t iop, PhidgetNetConnHandle nc, netreq_t *req) {
	...
	default:
		...
		return (MOS_ERROR(iop, EPHIDGET_UNEXPECTED, "unknown command msg stype:%x", req->nr_stype));
	}
}
...
handleDeviceClientRequest(mosiop_t iop, PhidgetNetConnHandle nc, netreq_t *req, int *stop) {
	...
	return (MOS_ERROR(iop, EPHIDGET_UNEXPECTED, "unknown msg type:%x", req->nr_type));
}

PhidgetReturnCode
openNetworkChannel(PhidgetChannelHandle channel, PhidgetDeviceHandle device, int uniqueIndex) {
	...
	len = mkJSON(netConn->nc->databuf, netConn->nc->databufsz,
	  ...
	  netConn->id, PHIDID(channel), channel->class, uniqueIndex, def->version);
	...
	if (wfr->req.nr_type == MSG_COMMAND) {
		PhidgetRelease(&netConn);
		res = simpleWaitForReply(&wfr, &rres, NULL);
		if (res != EPHIDGET_OK)
			return (res);
		return (rres);
	}
...
runClient(void *arg) {
	...
	netlogdebug("client thread started for '%s'", nc->peername);
	...
	if (stop == 0 && res != EPHIDGET_OK)
		netlogerr("clientHandleMessage() for %s failed:%s\n\t%N", nc->peername, mos_notice_name(res), iop);
