...
static void
PhidgetUSBConnectionDelete(PhidgetUSBConnectionHandle *conn) {
	...
	mos_free(*conn, sizeof(PhidgetUSBConnection));
}

PhidgetReturnCode
PhidgetUSBConnectionCreate(PhidgetUSBConnectionHandle *conn) {

	assert(conn);

	*conn = mos_zalloc(sizeof(PhidgetUSBConnection));
	phidget_init((PhidgetHandle)*conn, PHIDGET_USB_CONNECTION, (PhidgetDelete_t)PhidgetUSBConnectionDelete);
#ifdef _WINDOWS
	...
	if ((!(*conn)->asyncRead.hEvent) || (!(*conn)->asyncWrite.hEvent) || (!(*conn)->closeReadEvent)) {
		...
		logerr("CThread_create_event failed with error code: 0x%08x", errorcode);
		return (EPHIDGET_UNEXPECTED);
	}
#endif
...
PhidgetUSBReadThreadFunction(void *_device) {
	...
	loginfo("%"PRIphid": ReadThread starting", _device);
	...
	while (ISATTACHED(device)) {
		...
		default:
			logerr("%"PRIphid": ReadThread exiting - PhidgetDevice_read() returned : 0x%08x", device, res);
			usbError = 1;
			goto exit;
		}
	}

	loginfo("%"PRIphid": ReadThread exiting normally (Phidget detached)", device);
...
openAttachedUSBDevice(PhidgetDeviceHandle device) {
	...
	result = PhidgetUSBOpenHandle(device);
	...
	if ((result = device->initAfterOpen(device)) != 0) {
		logerr("Device Initialization functions failed: 0x%08x", result);
