...
vector<string> gps_simul_front, gps_simul_back;
int t = 0;
int simul_length_front = 0, simul_length_back = 0;
////////////////////////////////////////////////////
...
int main()
{
	...
	string input = "";
        ...
        gps_simul_front.clear();
        while(input.find("%") == string::npos){
                cin >> input;
                gps_simul_front.push_back(input);
        }
        simul_length_front = gps_simul_front.size();

        input = "";

        gps_simul_back.clear();
        while(input.find("%") == string::npos){
                cin >> input;
                gps_simul_back.push_back(input);
        }
        simul_length_back = gps_simul_back.size();

	char * cstr = new char [gps_simul_front[0].length()+1];
        std::strcpy (cstr, gps_simul_front[0].c_str());
        parse(cstr, gps_data);
        delete cstr;
...
void alarmWakeup(int sig_num)
{
        if(sig_num == SIGALRM) {
                ...
                /////////////////////fetch gps data///////////////
		if((systemState & HEADING_FRONT) == HEADING_FRONT){
//			printf("front gps fetched\n");
	                if(t < simul_length_front){
				char * cstr = new char [gps_simul_front[t].length()+1];
				std::strcpy (cstr, gps_simul_front[t].c_str());
				parse(cstr, gps_data);
				delete cstr;
			}
		}else if((systemState & HEADING_BACK) == HEADING_BACK){
//			printf("back gps fetched\n");
			if(t < simul_length_back){
                                char * cstr = new char [gps_simul_back[t].length()+1];
                                std::strcpy (cstr, gps_simul_back[t].c_str());
                                parse(cstr, gps_data);
				delete cstr;
                        }
		}
...
void update_distance(int encoder, Temperature left, Temperature right){
	...
	/*
	DrivingData temp;
        temp.ll.lat = gps_data.latitude;
        temp.ll.lng = gps_data.longitude;
        temp.utm.x = utm_data.x;
        temp.utm.y = utm_data.y;
        temp.distance = travelDistance;
        temp.l_temp = left;
        temp.r_temp = right;
	*/
...
void update_distance_back(int encoder, DrivingData& current){
	...
	/*
	current.ll.lat = gps_data.latitude;
        current.ll.lng = gps_data.longitude;
        current.utm.x = utm_data.x;
        current.utm.y = utm_data.y;
        current.distance = travelDistance;
	*/
...
void sortByRightTemparature(vector<DrivingData> &object){
	sort(object.begin(), object.end(), [] (DrivingData const& a, DrivingData const& b) { return a.r_temp < b.r_temp; });
}

void sortByLeftTemparature(vector<DrivingData> &object){
        sort(object.begin(), object.end(), [] (DrivingData const& a, DrivingData const& b) { return a.l_temp < b.l_temp; });
}
